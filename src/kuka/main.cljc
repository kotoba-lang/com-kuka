(ns kuka.main
  "Kotodama WASM entrypoint for the Kuka clean-room actor (L4) — Clojure port.
  ...
  facts (`kuka.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"kuka.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kuka-compat" :tier tier :entities entities} 200])
